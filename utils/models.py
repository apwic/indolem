...
class Model(nn.Module):
    def __init__(self, args, device, lang2model):
    ...
    def prediction(self, dataset):
        ...
        for j in range(0, len(dataset), self.args.batch_size):
            src, seg, label, mask_src = Batch(dataset, j, self.args.batch_size, self.args.device).get()
    ...
    def train(self, train_dataset, dev_dataset, test_dataset, logger):
        """ Train the model """
        ...
        logger.info("  Warming up = %d", self.args.warmup_steps)
        ...
        for i in range(int(self.args.num_train_epochs)):
            ...
            for j in range(0, len(train_dataset), self.args.batch_size):
                src, seg, label, mask_src = Batch(train_dataset, j, self.args.batch_size, self.args.device).get()
                self.train()
